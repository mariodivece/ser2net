...
internal static partial class LoggerExtensions
{
    ...
    internal static partial void LogListenerStarted(this ILogger<NetServer> logger, EndPoint endPoint);

    [LoggerMessage(LogLevel.Critical, "[TCP Server] Server on [{EndPoint}] irrecoverably failed.\r\n{Message}")]
    internal static partial void LogListenerFailed(this ILogger<NetServer> logger, EndPoint? endPoint, string message, Exception ex);

    [LoggerMessage(LogLevel.Critical, "[TCP Server] Server on [{EndPoint}] failed to listen for connections.\r\n{Message}")]
    internal static partial void LogListenerLoopFailed(this ILogger<NetServer> logger, EndPoint? endPoint, string message, Exception ex);

    [LoggerMessage(LogLevel.Information, "[TCP Server] Server on [{EndPoint}] is shutting down.")]
    internal static partial void LogListenerShuttingDown(this ILogger<NetServer> logger, EndPoint? endPoint);

    [LoggerMessage(LogLevel.Information, "Client [{EndPoint}] Connection accepted.")]
    internal static partial void LogClientAccepted(this ILogger<NetServer> logger, EndPoint endPoint);

    [LoggerMessage(LogLevel.Warning, "Client [{EndPoint}] rejected because connection count would exceed {MaxConnections}.")]
    internal static partial void LogConnectionRejectedMax(this ILogger<NetServer> logger, EndPoint endPoint, int maxConnections);

    [LoggerMessage(LogLevel.Warning, "Client [{EndPoint}] did not complete the connection.")]
    internal static partial void LogConnectionNotCompleted(this ILogger<NetServer> logger, EndPoint endPoint);
